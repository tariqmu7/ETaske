...
// The left column is English source text = the i18next key; the key names in
// the right column ("g t", "Esc") are literal keyboard keys and never translate.
const GROUPS: { title: string; rows: [string, string][] }[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['g k', 'Clients'],
      ['g w', 'Waiting on us / them'],
      ['g l', 'Deadline calendar'],
      ['g f', 'Documents'],
      ['g m', 'Meetings'],
      ['g r', 'Handover file'],
      ['g d', 'Needs you today'],
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid var(--border)', background: 'var(--surface-3)' }}>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: 'var(--text-primary)' }}>{t('Keyboard shortcuts')}</h3>
        ...
        <div style={{ padding: 18, display: 'grid', gap: 20 }}>
          {GROUPS.map(group => (
            <div key={group.title}>
              <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>{t(group.title)}</div>
              <div style={{ display: 'grid', gap: 8 }}>
                {group.rows.map(([keys, desc]) => (
                  <div key={keys} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                    <span style={{ fontSize: 13, color: 'var(--text-primary)' }}>{t(desc)}</span>
